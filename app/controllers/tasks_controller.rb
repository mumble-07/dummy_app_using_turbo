  ...
  def destroy
    ...
    if @task.destroy
      ...
      respond_to do |format|
        format.html { redirect_to category_tasks_path(@task.category_id), notice: 'Task was successfully destroyed.' }
        format.json { head :no_content }
        format.turbo_stream { render turbo_stream: turbo_stream.remove(dom_id(@task.category_id)) }
      end
    else
      render :index
    end
